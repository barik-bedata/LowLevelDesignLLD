...
* 💡 **কনসেপ্ট ক্লিয়ারিং ও ইন্টারভিউ লজিক (Candidate Key & Minimality Deep Dive)**:
  - **প্রশ্ন: ক্যান্ডিডেট কী ১টি কলামের হলে, একই টেবিলে ২টি কলামের আরেকটি ক্যান্ডিডেট কী কীভাবে থাকতে পারে? "Minimal" মানে তো সর্বনিম্ন বা ১টি কলাম হওয়ার কথা, তাহলে ২ কলামের কী কীভাবে Minimal হয়?**
    ...
    - **পার্থক্য বোঝার উদাহরণ**:
      ধরি, একটি গাড়ির রেজিস্ট্রেশন টেবিল আছে: `vehicles(vehicle_id, registration_no, state_code, license_plate_no, owner_name)`।
      ১. **`{vehicle_id}`** ইউনিক, তাই এটি সুপার কী। এটি ১ কলামের এবং এর থেকে কোনো কলাম বাদ দেওয়া যায় না। তাই এটি ক্যান্ডিডেট কী।
      ২. এবার **`{state_code, license_plate_no}`** সেটটি লক্ষ্য করুন। এটিও ইউনিক (সুপার কী), কারণ পুরো দেশে একটি নির্দিষ্ট স্টেটে একটি লাইসেন্স প্লেট নম্বর কেবল একটি গাড়িরই থাকতে পারে। কিন্তু ভিন্ন ভিন্ন স্টেটে একই লাইসেন্স প্লেট নম্বর থাকতে পারে (যেমন: California-তে প্লেট `XYZ-123` থাকতে পারে, আবার New York-এও প্লেট `XYZ-123` থাকতে পারে)।
      ৩. এবার মিনিমালিটি পরীক্ষা করার জন্য এর উপাদানগুলো একে একে বাদ দিয়ে দেখি:
        - `state_code` বাদ দিলে থাকে `{license_plate_no}` $\rightarrow$ এটি কি ইউনিক? **না** (কারণ একাধিক স্টেটে একই প্লেট নম্বর থাকতে পারে)।
        - `license_plate_no` বাদ দিলে থাকে `{state_code}` $\rightarrow$ এটি কি ইউনিক? **না** (একই স্টেটের হাজার হাজার গাড়ি আছে)।
      ৪. যেহেতু `{state_code, license_plate_no}` সেটের ভেতর থেকে কোনো একটি কলামও বাদ দিলে তা আর ইউনিক থাকে না, তাই এই জোড়াটি **স্থানীয়ভাবে অবিভাজ্য বা Minimal**। 
      ৫. টেবিলে ১ কলামের `{vehicle_id}` থাকা সত্ত্বেও, `{state_code, license_plate_no}` জোড়াটি নিজে থেকে অবিভাজ্য হওয়ায় এটিও একটি ১০০% বৈধ ক্যান্ডিডেট কী।
    - **অফিসিয়াল টেক্সটবুক রুল (Minimality Rule)**:
      ...
      যেহেতু `{vehicle_id}` সেটটি `{state_code, license_plate_no}` সেটের সাবসেট (subset) নয়, তাই `{vehicle_id}` এর অস্তিত্ব `{state_code, license_plate_no}` এর ক্যান্ডিডেট কী হওয়াতে কোনো বাধা সৃষ্টি করে না।
