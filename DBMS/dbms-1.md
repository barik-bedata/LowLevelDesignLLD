...
* 💡 **কনসেপ্ট ক্লিয়ারিং ও ইন্টারভিউ লজিক (Candidate Key & Minimality Deep Dive)**:
  - **প্রশ্ন: ক্যান্ডিডেট কী ১টি কলামের হলে, একই টেবিলে ২টি কলামের আরেকটি ক্যান্ডিডেট কী কীভাবে থাকতে পারে? "Minimal" মানে তো সর্বনিম্ন বা ১টি কলাম হওয়ার কথা, তাহলে ২ কলামের কী কীভাবে Minimal হয়?**
    ...
    - **পার্থক্য বোঝার উদাহরণ**:
      ধরি, একটি কর্মচারী টেবিল আছে: `employees(emp_id, first_name, last_name, salary)`।
      ১. `{emp_id}` ইউনিক, তাই এটি সুপার কী। এটি ১ কলামের এবং এর থেকে কিছু বাদ দেওয়া যায় না। তাই এটি ক্যান্ডিডেট কী।
      ২. এবার `{first_name, last_name}` সেটটি লক্ষ্য করুন। এটিও ইউনিক (সুপার কী)। আমরা যদি এর থেকে `first_name` বাদ দিই, তবে বাকি থাকে `{last_name}` যা ইউনিক নয়। আবার যদি `last_name` বাদ দিই, তবে থাকে `{first_name}` যা ইউনিক নয়। 
      ৩. যেহেতু `{first_name, last_name}` সেটের ভেতর থেকে কোনো একটি কলামও বাদ দিলে তা আর ইউনিক থাকে না, তাই এই জোড়াটি **স্থানীয়ভাবে অবিভাজ্য বা Minimal**। 
      ৪. টেবিলে ১ কলামের `{emp_id}` থাকা সত্ত্বেও, `{first_name, last_name}` জোড়াটি নিজে থেকে অবিভাজ্য হওয়ায় এটিও একটি বৈধ ক্যান্ডিডেট কী।
    - **অফিসিয়াল টেক্সটবুক রুল (Minimality Rule)**:
      ...
      যেহেতু `{emp_id}` সেটটি `{first_name, last_name}` সেটের সাবসেট (subset) নয়, তাই `{emp_id}` এর অস্তিত্ব `{first_name, last_name}` এর ক্যান্ডিডেট কী হওয়াতে কোনো বাধা সৃষ্টি করে না।
